...
const organizationSchema = {
  ...
  "@graph": [
    ...
    { "@type": "ProfessionalService", "@id": `${siteConfig.domain}/#service`, name: siteConfig.name, url: siteConfig.domain, email: siteConfig.email, telephone: siteConfig.whatsapp.displayNumber, areaServed: { "@type": "Country", name: "United Kingdom" }, serviceType: services.map((service) => service.title), hasOfferCatalog: { "@type": "OfferCatalog", name: "Digital services", itemListElement: services.map((service) => ({ "@type": "Offer", itemOffered: { "@type": "Service", name: service.title, url: `${siteConfig.domain}/services/${service.slug}` } })) } },
